...
4. **Apply the detector-driven-change probe.** For each *Detector hits* entry (`sops/dev-pipeline.md` § PR Body), open the flagged location and compare that file's diff with the detector's own finding: the diff should touch the flagged line and do what the resolution says. Then run `git log -p origin/<default-branch>..HEAD` over the changed fixture and test files. Text rewritten with no behaviour change, or rewritten more than once, is a detector hit and needs an entry. For each hit, ask whether the same change would appear in a PR where the detector didn't exist. If not, it is a `warning`. A hit with no entry is a `warning`. **Blockers only inside the model.** Beyond the probe above, a finding of any severity blocks only if it is (1) reachable under the issue's stated threat model (`sops/dev-pipeline.md` § Threat model in the issue), (2) a regression of behaviour that worked before this PR, or (3) a claim the PR body, docs, or code comments make that the code does not actually do. Everything else is real but out of the model — record it under *Known limits, for an issue* instead of blocking, and the PR can merge. #34's rounds 3–5 spent real review time on constructions the issue never said were in scope (a deliberate binary-file evasion, a crafted phantom file header, a self-comparing filter check); #48's round 3 correctly blocked a widening that excused real hyphenated passphrases, because accidental leaks — not only deliberate evasion — were squarely in `leak-check`'s stated model (#42 F1).
