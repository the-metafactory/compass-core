...
- **Stop after two rounds.** After two `changes-requested` rounds on one PR (`sops/pr-review.md` § Severity → Verdict), stop and decide — before dispatching a third — one of: re-plan the slice, split the PR, or bring it to the principal. A third round is a decision, not a default, and the decision gets recorded on the PR. #34 ran five request-changes rounds with no such checkpoint recorded. Round 1 (F1) blocked on a real regression, and rounds 3–5 each also carried a false-claim blocker under Severity → Verdict § 4's criterion 3 (H3, J3, K1) — but the deliberate-evasion constructions in rounds 3 and 4 (H1's NUL-byte exemption, J1's phantom file header) would have been recorded as `info` under A2, not blockers, had #32 stated a threat model up front.
